...
class R2DBCPaymentDatabaseHelper (
    ...
    companion object {
        private val SELECT_PAYMENT_QUERY = """
          SELECT * FROM payment_events pe 
          INNER JOIN payment_orders po ON pe.order_id = po.order_id
          WHERE pe.order_id = :orderId
        """.trimIndent()

        private val DELETE_PAYMENT_EVENT_QUERY = """
          DELETE FROM payment_events
        """.trimIndent()

        private val DELETE_PAYMENT_ORDER_QUERY = """
          DELETE FROM payment_orders
        """.trimIndent()

        private val DELETE_PAYMENT_ORDER_HISTORY_QUERY = """
          DELETE FROM payment_order_histories
        """.trimIndent()

        private val DELETE_OUTBOX_QUERY = """
