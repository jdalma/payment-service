...
class R2DBCPaymentRepository (
    ...
    private fun selectPaymentEventId(): Mono<Long> = databaseClient.sql(LAST_INSERT_ID_QUERY)
        .fetch()
        .first()
    ...
    companion object {
        private val INSERT_PAYMENT_EVENT_QUERY = """
          INSERT INTO payment_events (buyer_id, order_name, order_id)
          VALUES (:buyerId, :orderName, :orderId) 
        """.trimIndent()

        private val LAST_INSERT_ID_QUERY = """
          SELECT LAST_INSERT_ID()
        """.trimIndent()

        private val INSERT_PAYMENT_ORDER_QUERY = fun (valueClauses: String) = """
          INSERT INTO payment_orders (payment_event_id, seller_id, order_id, product_id, amount, payment_order_status) 
          VALUES $valueClauses
        """.trimIndent()

        // EXECUTING 상태는 처리 지연 시간이 3분 초과한 결제건만 조회한다.
        private val SELECT_PENDING_PAYMENT_QUERY = """
